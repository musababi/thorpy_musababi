...
if __name__ == '__main__':
    ...
    v_x = 10 # mm/s given random and constant
    v_y = 10
    w = 0
    ...
    x_end = 20
    y_end = -20
    x = np.linspace(x_init, x_end, int(abs(x_end - x_init)/interval +1))
    y = np.linspace(y_init, y_end, int(abs(y_end - y_init)/interval +1))
